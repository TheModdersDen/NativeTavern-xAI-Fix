...
CLEAN_POD_CACHE="${CLEAN_POD_CACHE:-false}"
...
# flutter pub cache repair  # skipped: hangs on slow network, pub get suffices
...
if [ -f "$PBXPROJ_PATH" ]; then
    ...
    # Keep the app target aligned with the Podfile and App Store requirements.
    sed -i '' -E "s/IPHONEOS_DEPLOYMENT_TARGET = [^;]+;/IPHONEOS_DEPLOYMENT_TARGET = ${IOS_DEPLOYMENT_TARGET};/g" "$PBXPROJ_PATH"
    echo "Set iOS deployment target to: $IOS_DEPLOYMENT_TARGET"
...
' "$INFO_PLIST_PATH"
    ...
    # Microphone + speech recognition (voice input / STT)
    if ! grep -q "NSMicrophoneUsageDescription" "$INFO_PLIST_PATH"; then
        sed -i '' '/<\/dict>/i\
	<key>NSMicrophoneUsageDescription</key>\
	<string>This app needs microphone access for voice input (speech-to-text).</string>\
	<key>NSSpeechRecognitionUsageDescription</key>\
	<string>This app uses speech recognition to convert your voice into chat messages.</string>
' "$INFO_PLIST_PATH"
        echo "Added microphone/speech recognition descriptions to Info.plist"
    else
        echo "Microphone descriptions already exist in Info.plist"
    fi
...
if [ "$BUILD_FOR_DEVICE" = "true" ]; then
    ...
    # Export the archive for App Store Connect upload.
    echo ""
...
EOF
    ...
    if [ -n "$TEAM_ID" ]; then
        ...
        if [ -n "$EXPORTED_IPA" ]; then
            ...
            cp "$EXPORTED_IPA" "release/${IPA_NAME}"
            echo "✓ IPA exported: release/${IPA_NAME}"
        else
            echo "App Store IPA export did not produce an .ipa file"
            exit 1
        fi
    fi
    ...
    echo ""
    echo "App Store IPA: release/${IPA_NAME}"
    echo ""
